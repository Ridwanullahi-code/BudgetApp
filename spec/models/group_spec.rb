...
RSpec.describe Group, type: :model do
  ...
  describe 'validations' do
    ...
    it 'is valid with a name and an icon' do
      group = Group.new(name: 'Test', icon: fixture_file_upload('icon.jpeg', 'image/jpeg'), author: user)
      expect(group).to be_valid
    end

    it 'is invalid without a name' do
      group = Group.new(icon: fixture_file_upload('icon.jpeg', 'image/jpeg'), author: user)
  ...
  describe '#total_amount' do
    it 'returns the sum of expenses amount for the group' do
      group = Group.create(name: 'Test', icon: fixture_file_upload('icon.jpeg', 'image/jpeg'), author: user)
      expense1 = Expense.create(name: 'text expense 1', amount: 10, author: user)
